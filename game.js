...
function renderFooterLinks() {
    return '<footer class="footer-links"><a href="/projects/">← Projects</a><a href="https://github.com/tkwa/save-the-world-game" target="_blank" rel="noopener">Source code</a><span>Critical Path · campaign preview</span></footer>';
}
function renderMetrics(forecast) {
    ...
    return `<div class="metrics" aria-label="Campaign indicators">
      <div class="metric"><span class="metric-label">AI capability</span><span class="metric-value number">${number(campaign.player.capability,1)}×</span><span class="metric-detail">${lead >= 1 ? `${number(lead,1)}× ahead of nearest rival` : `${number(1/lead,1)}× behind the leader`}</span></div>
...
function renderEnding() {
    ...
    return `<main id="main" class="ending-shell">${renderNotice()}<header class="ending-heading"><div class="eyebrow">${escape(String(outcome.year))} · ${escape(outcome.winner || 'The transition')}</div>${heading(titles[outcome.kind] || 'Beyond the transition',1,'ending-title')}<p>${escape(outcome.summary)}</p></header>
      ...
      <div class="ending-details"><section><h3>What shaped this outcome</h3><ul class="cause-list">${(outcome.causes || []).map(cause => `<li>${escape(cause)}</li>`).join('')}</ul><p class="small muted" style="margin-top:20px">Estimated existential risk at the transition: ${percent(outcome.transitionRisk,1)}. This is one realized future, not a guarantee or an overall score.</p></section><section><h3>Beyond the transition</h3><div class="milestones">${[
